#' Estimate parameters of the assemblage.
#'
...
#'
#'@param data Dataframe giving P/A or abundance (counts or coverage) of species.
...
#'
#'@param spatial.str Logical argument indicating whether spatial information
#'  (coordinates) of samples is available. If set TRUE, a matrix or dataframe
#'  with X and Y coordinates must be supplied. This file must have the same
#'  number of rows than data to be used by the function Moran.I{ape}. The
#'  analisys is only valid for individual counts (argument type = "counts"), and
#'  will gave an Error if two or more samples share the same X and Y values.
#'@param coords Matrix or dataframe given Y and X coordinates of samples.
#'@param moranI.alfa Probability for rejecting random spatial distribution. Default is < 0.05.
#'@param spatial.unsee.sp Spatial distribution to be assumed for unseen species. It might be "random", "positive" or "negative".
#'
#' @return The function returns an object of class List, to be used by SimDat.
#' @export
#'
#' @examples
